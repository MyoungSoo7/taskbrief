...
> 스크린샷과 데모 GIF는 준비 중입니다. 아래 [아키텍처](#아키텍처) 다이어그램은 GitHub에서 바로 렌더링됩니다.

<!-- 캡처 후 아래 주석(<!-- -->)을 제거하면 표시됩니다. 파일은 docs/images/ 에 저장 — 캡처 가이드: docs/images/README.md
### Swagger UI
![Swagger UI](docs/images/swagger-ui.png)

### 일일 브리핑 응답
![일일 브리핑 응답](docs/images/briefing-response.png)
...
flowchart TD
    ...
    subgraph R["routers · HTTP 관심사"]
        direction LR
        A1["auth.py<br/>/auth/*"]
        A2["tasks.py<br/>/tasks CRUD"]
        A3["briefing.py<br/>/briefing/*"]
    end

    subgraph S["services · 비즈니스 로직"]
        direction LR
        S1["task_service.py"]
        S2["briefing_service.py<br/>오케스트레이션·캐싱"]
        S3["ai_service.py<br/>LLM 호출 격리"]
    end
    ...
    S3 -. "Claude API" .-> LLM[("Anthropic API")]
```
...
sequenceDiagram
    ...
    participant R as briefing 라우터
    participant B as briefing_service
    participant DB as DB
    participant AI as ai_service → Claude

    U->>R: GET /briefing/daily (JWT)
    R->>B: get_briefing(user, "daily")
    B->>DB: 대상 할일 조회

    alt 할일 0개
        B-->>U: 고정 메시지 (LLM 호출 없음)
    else 캐시 유효 (같은 로컬 날짜 + 같은 fingerprint)
        B->>DB: 최신 브리핑 조회
        B-->>U: 캐시 반환 (cached=true)
    else 캐시 무효
        B->>AI: generate_briefing(tasks)
        AI-->>B: summary + suggestions (Pydantic 검증)
        B->>DB: Briefing 저장
        B-->>U: 새 브리핑 (cached=false)
    end
```
...
총 **49개** 테스트가 통과한다. `ai_service`가 mock으로 대체되므로 **`ANTHROPIC_API_KEY` 없이도 전체 테스트가 통과**한다. 테스트 DB는 in-memory SQLite로 테스트별로 격리된다. 커버리지는 인증 플로우, 데이터 격리(404), CRUD 및 필터·페이지네이션 경계값, 브리핑(정상·타임아웃·파싱 실패 503·할일 0개·캐시 히트·캐시 무효화)을 포함한다.
